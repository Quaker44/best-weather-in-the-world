...
WEATHERSCORE_FRSHTT_RULES = {
    ...
    2: 0,   # Effect for the second digit being 1
...
# Parameters for second digit adjustments
SECOND_DIGIT_THRESHOLD_UPPER = 183  # Upper threshold for rainy days
SECOND_DIGIT_THRESHOLD_LOWER = 26  # Lower threshold for rainy days
SECOND_DIGIT_ADJUSTMENT = 30       # Adjustment value for second digit conditions
...
def calculate_average_weatherscore(csv_file):
    ...
    with open(csv_file, mode='r') as file:
        ...
        for row in csv_reader:
            ...
            # Check the second digit of FRSHTT
            if frshtt[1] == '1':
                second_digit_1_count += 1

            weatherscore = calculate_weatherscore(heat_index, wdsp, frshtt)
            total_scores += weatherscore

        # Adjust total_scores based on the count of the second digit being 1
        if second_digit_1_count > SECOND_DIGIT_THRESHOLD_UPPER:
            total_scores += (second_digit_1_count - SECOND_DIGIT_THRESHOLD_UPPER) * SECOND_DIGIT_ADJUSTMENT
        elif second_digit_1_count < SECOND_DIGIT_THRESHOLD_LOWER:
            total_scores += (SECOND_DIGIT_THRESHOLD_LOWER - second_digit_1_count) * SECOND_DIGIT_ADJUSTMENT

        # Check conditions to skip the file
        if all_frshtt_zero or num_rows < 329:
            return None
...
def save_annual_weatherscore(csv_file, output_file, file_number):
    ...
    if annual_weatherscore is None:
        print(f"Skipping file {csv_file} due to incomplete data.")
        return
...
output_file = '/Users/Quaker/Desktop/Weather_Station_Project/annual_weatherscore_output(4).csv'
